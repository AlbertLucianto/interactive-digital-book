...
"Mac users be aware that osX and iOS can't use multiple microphones at once! This is a limitation in the OS and is not bound to Unity or MicControl2"
" This means that Mac users are always limited to one microphone input. This makes it virtually impossible to create multiplayer microphone controlled games on a Mac."
" If you really need multiple microphones setup for interaction in your game, I am afraid your only option is to move your development to the Windows os (or run it in bootcamp).

~~ How To Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
For a single scene:
1. Go to GameObject-Audio-MicControl.
2. The controller will be placed in the Scene.
3. You can now use "YourVariable.GetComponent(MicControl).loudness" in your scripts to receive the device's loudness' 
   (look in the Code reference folder for a_callReference to read more info);.

For multiple scenes:
1. Create an empty scene (this is to load a single persistent controller).
2. Go to GameObject-Audio-MicControl.
3. The controller will be placed in the Scene.
4. On the controller expand the advanced settings tab and select "Don't destroy on load".
5. In your new script you must first use void/function start () to dynamically find the persistent controller in your scripts: yourVariable = GameObject.Find("YourControllerName");
6. You can now use "yourVariable.GetComponent(MicControl).loudness" in your scripts to receive the device's loudness' 
   (look in the Code reference folder for a_callReference to read more info);.
...
var spectrumData:	This is the bigger brother of the "loudness" variable. Instead of converting everything to one single float value, users have acces to the full frequency spectrum recorded/streamed
					by the microphone.
...
//call these functions in external scripts to start or stop the microphone (not recommended as it may distort your data stream, only use if really needed).
public functions StartMicrophone () -
This starts the listening of audio from your microphone. Alternativly if you want a complete fresh initialisation of your device, use the "Start()" function instead.

public functions StopMicrophone () - 
This stops the playing of audio from your microphone.


//these will be handled automatically by the system and should never be manually called.
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
"~~ Chapter 3: WebPlayer Setup (THIS IS NOW DEPRICATED AND NO LONGER SUPPORTED)~~
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
